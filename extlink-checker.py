...
def check(args: argparse.Namespace, api: API, db: Database) -> None:
...
def update(args: argparse.Namespace, api: API, db: Database) -> None:
    # create updater and add checkers
    updater = Updater.from_argparser(args, api)
    checker = ExtlinkStatusUpdater(api, db)
