...
class MediaWikiFixtureInstance:
    def __init__(
        ...
        with self.db_autocommit_engine.connect() as conn:
            databases = list(conn.execute(sa.text("SELECT datname FROM pg_database")).scalars())
        ...
        if self.db_name + "_template" not in databases:
            ...
            self._create_database(name=self.db_name + "_template", template=self.db_name)
    ...
    def _drop_database(self, name=None):
        ...
        with self.db_autocommit_engine.connect() as conn:
            ...
            conn.execute(sa.text(f"UPDATE pg_database SET datallowconn=false WHERE datname = '{name}'"))
            try:
                conn.execute(sa.text(f"SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '{name}'"))
            except sa.exc.OperationalError:
        ...
        with self.db_autocommit_engine.connect() as conn:
            ...
            conn.execute(sa.text(f"UPDATE pg_database SET datallowconn=true WHERE datname = '{name}'"))
    ...
    def _create_database(self, name=None, template=None):
        ...
        with self.db_autocommit_engine.connect() as conn:
            ...
            conn.execute(sa.text(f"UPDATE pg_database SET datallowconn=false WHERE datname = '{template}'"))
            try:
                conn.execute(sa.text(f"SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '{template}'"))
            except sa.exc.OperationalError:
                # the psycopg driver raises an exception when the command terminates the current connection
                pass

        with self.db_autocommit_engine.connect() as conn:
            conn.execute(sa.text(f"CREATE DATABASE {name} WITH TEMPLATE {template} OWNER {self.db_user}"))
            conn.execute(sa.text(f"UPDATE pg_database SET datallowconn=true WHERE datname = '{template}'"))
...
def mediawiki(
    ...
    assert db_name
    assert db_user
    instance = MediaWikiFixtureInstance(mediawiki_service, user, password, mediawiki_database_url, db_name, db_user)
    yield instance
    instance.clear()
