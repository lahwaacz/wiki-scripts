...
class test_connection:
    ...
    def test_coverage(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
    ...
    def test_url(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        assert api.api_url == f"{mediawiki.url}/api.php"

    def test_params_is_dict(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        with pytest.raises(ValueError):
            api.call_api(params=("foo", "bar"))

    def test_wrong_action(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        with pytest.raises(APIWrongAction):
            api.call_api(params={"action": "wrong action"})

    def test_empty_query_expand(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        with pytest.raises(APIExpandResultFailed):
            api.call_api(action="query")

    def test_empty_query(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        expected_response = {"batchcomplete": ""}
        assert api.call_api(action="query", expand_result=False) == expected_response

    def test_post_action(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        with pytest.raises(APIError):
            api.call_api(action="stashedit")

    def test_help(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        h = api.call_api(action="help")
        assert h["mime"] == "text/html"
        assert isinstance(h["help"], str)

    def test_mixed_params_and_kwargs(self, mediawiki: MediaWikiFixtureInstance):
        api = mediawiki.api
        with pytest.raises(ValueError):
