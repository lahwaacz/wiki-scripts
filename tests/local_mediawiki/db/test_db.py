...
def tag_revisions(mediawiki: MediaWikiFixtureInstance, tag: str, title: str) -> None:
    revids = _api_get_revisions_of_page(mediawiki.api, title) + _api_get_deleted_revisions_of_page(mediawiki.api, title)
    assert revids
    params = {
        "action": "tag",
        "revid": "|".join(revids),
...
def _check_alldeletedrevisions(mediawiki: MediaWikiFixtureInstance, db: Database) -> None:
    prop = {
        "ids",
        "flags",
        "timestamp",
