...
class test_encodings:
    ...
    def test_encode_chars(self) -> None:
    ...
    def test_urldecode_noop(self) -> None:
        for s in [string.ascii_letters, self.ascii_all, self.unicode_sample]:
            dec = urldecode(s)
            assert dec == s

    def test_queryencode(self) -> None:
        skipped = string.ascii_letters + string.digits + "-_."
        for s in [self.ascii_all, self.unicode_sample]:
            e1 = queryencode(s)
            e2 = urllib.parse.quote(s, safe=skipped)
    ...
    def test_querydecode(self) -> None:
        for s in [string.ascii_letters, self.ascii_all, self.unicode_sample]:
            enc = queryencode(s)
            dec = querydecode(enc)
            assert dec == s

    def test_dotencode_basic(self) -> None:
        skipped = string.ascii_letters + string.digits + "-_.:"
    ...
    def test_dotencode_spaces(self) -> None:
        s = " Foo   bar "
        e = "Foo_bar"
        assert dotencode(s) == e

    def test_dotencode_underscores(self) -> None:
        s = "_Foo__  __bar_"
        e = "Foo_bar"
        assert dotencode(s) == e

    def test_dotencode_whitespace(self) -> None:
        s = " Foo \t\t  bar "
        e = "Foo_.09.09_bar"
        assert dotencode(s) == e

    def test_dotencode_colons(self) -> None:
        s = " :: :: Foo bar"
        e = "_::_Foo_bar"
        assert dotencode(s) == e

    def test_dotencode_T20431(self) -> None:
    ...
    def test_anchorencode_legacy(self) -> None:
        for s in [
            string.ascii_letters,
            string.digits,
            string.punctuation,
    ...
    def test_anchorencode_html5(self) -> None:
        for s in [
            string.ascii_letters,
            string.digits,
            string.punctuation,
    ...
    def test_anchorencode_html5_separator(self) -> None:
    ...
    def test_anchorencode_html5_whitespace(self) -> None:
        s = "A\tB\nC\fD\rE F"
        e = "A_B_C_D_E_F"
        assert anchorencode(s, format="html5") == e

    def test_anchorencode_html5_percent(self) -> None:
        # only "%" from percent-encoded octets get encoded
        s = "A%G%20%aB"
        e = "A%G%2520%25aB"
        assert anchorencode(s, format="html5") == e

    def test_anchorencode_html5_brackets(self) -> None:
        # MW incompatibility: output should be usable in MediaWiki links
        s = "[MATCH] SECTION OPTIONS"
        e = "%5BMATCH%5D_SECTION_OPTIONS"
        assert anchorencode(s, format="html5") == e

    def test_anchorencode_invalid_format(self) -> None:
