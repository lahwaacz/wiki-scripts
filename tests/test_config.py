...
class test_argtype_bool:
    ...
    def test_true(self, string: str) -> None:
        result = ws.config.argtype_bool(string)
        assert result is True

    @pytest.mark.parametrize("string", ["no", "off", "false", "0", "No", "FalSE", "OFF"])
    def test_false(self, string: str) -> None:
        result = ws.config.argtype_bool(string)
        assert result is False

    def test_cannot_be_converted(self) -> None:
        string = "hello"
        msg = "cannot be converted to boolean"
        with pytest.raises(argparse.ArgumentTypeError) as excinfo:
            _ = ws.config.argtype_bool(string)
        assert msg in str(excinfo.value)

    @pytest.mark.parametrize("value", [True, 4, 5.5, ["hello", "world"]])
    def test_not_str_passed(self, value: Any) -> None:
        msg = "object has no attribute 'lower'"
        with pytest.raises(AttributeError) as excinfo:
            _ = ws.config.argtype_bool(value)
        assert msg in str(excinfo.value)
...
class test_argtype_config:
    @pytest.mark.parametrize(
        "string",
        [ws.config.DEFAULT_CONF, "archwiki", "wiki.archlinux.org", "hello-world.py"],
    )
    def test_configuration_name(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, string: str) -> None:
    ...
    def test_path_with_slashes_but_without_conf_suffix(self, tmp_path: Path, string: str) -> None:
        config = tmp_path / string
        with pytest.raises(argparse.ArgumentTypeError) as excinfo:
            _ = ws.config.argtype_config(config)
    ...
    def test_nonexisting_file(self, tmp_path: Path) -> None:
...
class test_argtype_existing_dir:
    """Tests for 'argtype_existing_dir()' function."""
    ...
    def test_nonexising_dir(self, tmp_path: Path) -> None:
        directory = tmp_path / "non-existing-dir"
        with pytest.raises(argparse.ArgumentTypeError) as excinfo:
            _ = ws.config.argtype_existing_dir(directory)
...
class test_argtype_dirname_must_exist:
    ...
    def test_filepath_with_existing_dirname(self, tmp_path: Path) -> None:
    ...
    def test_filepath_with_nonexisting_dirname(self, tmp_path: Path) -> None:
        existing_file = tmp_path / "subdir" / "not-a-file.txt"
        with pytest.raises(argparse.ArgumentTypeError) as excinfo:
            _ = ws.config.argtype_dirname_must_exist(existing_file)
        msg = "directory '%s' does not exist" % existing_file.parent
        assert msg == str(excinfo.value)


@pytest.fixture
def cfp(tmp_path: Path) -> ws.config.ConfigParser:
    data = """
    [DEFAULT]
    default_opt1 = value1
    default_opt2 =  value2
...
class test_fetch_section:
    ...
    def test_existent_section(self, cfp: ws.config.ConfigParser) -> None:
    ...
    def test_section_not_specified(self, cfp: ws.config.ConfigParser) -> None:
        values = cfp.fetch_section()
        result = ["--default_opt1", "value1", "--default_opt2", "value2"]
        assert values == result

    def test_multiline_value(self, cfp: ws.config.ConfigParser) -> None:
    ...
    def test_to_dict(self, cfp: ws.config.ConfigParser) -> None:
        values = cfp.fetch_section(to_list=False)
        result = {"default_opt1": "value1", "default_opt2": "value2"}
        assert values == result

    def test_short_option_in_config_file(self, cfp: ws.config.ConfigParser) -> None:
        with pytest.raises(argparse.ArgumentTypeError) as excinfo:
            _ = cfp.fetch_section(section="section3")
...
class test_object_from_argparser:
    ...
    def test_undefined(self, capsys: pytest.CaptureFixture) -> None:
    ...
    def test_defined_on_cli(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(sys, "argv", ["prog", "--foo", "a"])
        obj = ws.config.object_from_argparser(obj_simple)
        assert obj.foo == "a"

    def test_defined_in_config(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        config = tmp_path / "default.conf"
    ...
    def test_defined_in_both(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        config = tmp_path / "default.conf"
        with open(config, "w") as f:
            f.write("[DEFAULT]\nfoo = a\n")
    ...
    def test_unknown_config_argument(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        config = tmp_path / "default.conf"
        with open(config, "w") as f:
            f.write("[DEFAULT]\nunknown = value\n")
    ...
    def test_long_unknown_command_line_argument(self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture) -> None:
        monkeypatch.setattr(sys, "argv", ["prog", "--foo", "a", "--unknown", "value"])
        with pytest.raises(SystemExit) as excinfo:
            _ = ws.config.object_from_argparser(obj_simple)
        assert excinfo.type is SystemExit
        assert excinfo.value.code == 2
        assert "error: unrecognized arguments: --unknown" in capsys.readouterr().err

    def test_short_unknown_command_line_argument(self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture) -> None:
    ...
    def test_multiple_unknown_command_line_argument(self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture) -> None:
    ...
    def test_multiple_unknown_short_options_in_cli(self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture) -> None:
    ...
    def test_no_config(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        monkeypatch.setattr(ws.config, "CONFIG_DIR", tmp_path)
        config = tmp_path / "default.conf"
        with open(config, "w") as f:
            f.write("[DEFAULT]\nfoo = a\nbar = c\n")
    ...
    def test_no_config_side_by_side_with_config(
        self,
