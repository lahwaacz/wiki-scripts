...
class test_argtype_bool:
    ...
    @pytest.mark.parametrize(
        "string", ["no", "off", "false", "0", "No", "FalSE", "OFF"]
    )
...
class test_argtype_config:
    ...
    def test_configuration_name(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, string: str
    ) -> None:
    ...
    def test_path_with_slashes_but_without_conf_suffix(
        self, tmp_path: Path, string: str
    ) -> None:
...
class test_object_from_argparser:
    ...
    def test_defined_in_both(
        ...
        with open(config, "w") as f:
            f.write("[DEFAULT]\nfoo = a\n")
        monkeypatch.setattr(
            sys, "argv", ["prog", "--foo", "b", "--config", str(config)]
        )
        obj = ws.config.object_from_argparser(obj_simple)
        assert obj.foo == "b"

    def test_unknown_config_argument(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        config = tmp_path / "default.conf"
        with open(config, "w") as f:
            f.write("[DEFAULT]\nunknown = value\n")
        monkeypatch.setattr(
            sys, "argv", ["prog", "--foo", "a", "--config", str(config)]
        )
        obj = ws.config.object_from_argparser(obj_simple)
        assert obj.foo == "a"

    def test_long_unknown_command_line_argument(
    ...
    def test_short_unknown_command_line_argument(
        self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture
    ) -> None:
    ...
    def test_multiple_unknown_command_line_argument(
        self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture
    ) -> None:
        monkeypatch.setattr(
            sys, "argv", ["prog", "--foo", "a", "--unknown", "-u", "value"]
        )
        with pytest.raises(SystemExit) as excinfo:
            _ = ws.config.object_from_argparser(obj_simple)
        assert excinfo.type is SystemExit
    ...
    def test_multiple_unknown_short_options_in_cli(
        self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture
    ) -> None:
    ...
    def test_no_config_side_by_side_with_config(
        ...
        with open(config, "w") as f:
            f.write("[DEFAULT]\nfoo = a\nbar = c\n")
        monkeypatch.setattr(
            sys, "argv", ["prog", "--foo", "b", "--no-config", "--config", str(config)]
        )
        with pytest.raises(SystemExit) as excinfo:
            _ = ws.config.object_from_argparser(obj_simple)
        assert excinfo.type is SystemExit
        assert excinfo.value.code == 2
        assert (
            "error: argument -c/--config: not allowed with argument --no-config"
            in capsys.readouterr().err
        )
