...
class PkgFinder:
    def __init__(self, aurpkgs_url: str, tmpdir: str):
        self.aurpkgs_url = aurpkgs_url
        self.tmpdir = os.path.abspath(os.path.join(tmpdir, "wiki-scripts"))

        self.aurpkgs: set[str] | None = None
        self.pacdb = self.pacdb_init(
            PACCONF, os.path.join(self.tmpdir, "pacdbpath"), arch="x86_64"
        )

    def pacdb_init(self, config: str, dbpath: str, arch: str) -> pyalpm.Handle:
    ...
    def find_pkg(self, pkgname: str, exact: bool = True) -> pyalpm.Package | None:
    ...
    def find_grp(self, grpname: str, exact: bool = True) -> Any | None:
        for db in self.pacdb.get_syncdbs():
            if exact is True:
    ...
    def find_aur(self, pkgname: str) -> bool:
        if self.aurpkgs is None:
            return False
    ...
    def find_replaces(self, pkgname: str, exact: bool = True) -> pyalpm.Package | None:
        for db in self.pacdb.get_syncdbs():
            ...
            for pkg in db.pkgcache:
                if exact is True and pkgname in pkg.replaces:
                    return pkg
                elif exact is False and pkgname.lower() in (
                    _pkgname.lower() for _pkgname in pkg.replaces
                ):
                    return pkg
        return None


class PkgUpdater(ConfigurableObject):
    ...
    def __init__(
        ...
        # with wiki markup)
        self.log: dict[str, dict[str, list[str]]]
        self.log = {}
    ...
    def set_argparser(argparser: argparse.ArgumentParser) -> None:
        ...
        group.add_argument(
            "--aurpkgs-url",
            default="https://aur.archlinux.org/packages.gz",
            metavar="URL",
    ...
    def get_localized_template(self, template: str, lang: str = "English") -> str:
    ...
    def strip_whitespace(self, wikicode: Wikicode, template: Template) -> None:
        """
        ...
        template.name = str(template.name).strip()  # type: ignore
        param.value = param.value.strip()

    def update_package_template(
        self, template: Template, lang: str = "English"
    ) -> str | None:
        """
        ...
        :returns: A _hint_, which is either `None` if the template was updated successfully,
        ...
        if newtemplate is not None:
            ...
            if template.name.lower().strip() != newtemplate.lower():
                # (mwparserfromhell does not have getters and setters next to each other,
                # so mypy thinks the property is read-only)
                template.name = newtemplate  # type: ignore
    ...
    def update_page(self, title: str, text: str) -> Wikicode:
        """
        ...
        for template in wikicode.ifilter_templates():
            ...
            if not any(
                template.name.matches(tmp) for tmp in ["Aur", "AUR", "Grp", "Pkg"]
            ):
                continue
            ...
            if hint is not None:
                ...
                ensure_unflagged_by_template(
                    wikicode, template, "Broken package link", match_only_prefix=True
                )
                # flag with a localized template and hint
                flag = self.get_localized_template("Broken package link", lang)
                ensure_flagged_by_template(
                    wikicode, template, flag, hint, overwrite_parameters=True
                )
            else:
                ensure_unflagged_by_template(
                    wikicode, template, "Broken package link", match_only_prefix=True
    ...
    def add_report_line(self, title: str, template: str, message: str) -> None:
    ...
    def get_report_wikitext(self) -> str:
        report = ""
    ...
    def run(self) -> None:
        ...
        basename = os.path.join(
            self.report_dir, "update-pkgs-{}.report".format(timestamp)
        )
        ...
        if self.report_page:
            # (mwparserfromhell does not have getters and setters next to each other,
            # so mypy thinks the property is read-only)
            div.contents = mwreport  # type: ignore
            try:
...
class TemplateParametersError(Exception):
    ...
    def __init__(self, template: Template):
        self.message = (
            "Failed to parse a template parameter. This likely indicates a "
            "syntax error on the page.\n\n"
            f"Template text: '{template}'\n\n"
            f"Parsed parameters: {template.params}"
        )
...
if __name__ == "__main__":
    ...
    updater = ws.config.object_from_argparser(
        PkgUpdater, description="Update Pkg/AUR templates"
    )
    updater.run()
