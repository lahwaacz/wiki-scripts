...
class PkgUpdater(ConfigurableObject):
    ...
    def update_package_template(self, template: Template, lang: str = "English") -> str | None:
        """
    ...
    def update_page(self, title: str, text: str) -> Wikicode:
        ...
        for template in wikicode.ifilter_templates():
            ...
            if hint is not None:
                logger.warning(f"broken package link: {template}: {hint}")
        ...
        for ns in namespaces:
            for page in self.api.generator(
                ...
                if title in self.blacklist_pages:
                    logger.info(f"skipping blacklisted page [[{title}]]")
                    continue
    ...
    def add_report_line(self, title: str, template: str, message: str) -> None:
        message = f"<nowiki>{template}</nowiki> ({message})"
        lang = detect_language(title)[1]
    ...
    def save_report_to_json(self, text: str, basename: str) -> None:
        ...
        f.close()
        logger.info(f"Saved report in '{basename}.json'")

    def save_report_to_file(self, text: str, basename: str) -> None:
        f = open(basename + ".mediawiki", "w")
        f.write(text)
        f.close()
        logger.info(f"Saved report in '{basename}.mediawiki'")

    def run(self) -> None:
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d")
        basename = os.path.join(self.report_dir, f"update-pkgs-{timestamp}.report")
        ...
        if self.report_page:
            ...
            try:
                ...
                logger.info(f"Saved report to the [[{self.report_page}]] page on the wiki.")
            except APIError:
