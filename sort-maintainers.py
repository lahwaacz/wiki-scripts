...
class SortMaintainers(ConfigurableObject):
    ...
    def __init__(self, api: API, db: Database, pagename: str, days: int, min_edits: int):
        self.api = api
        self.db = db
    ...
    def run(self) -> None:
        ...
        try:
            ...
            logger.error(f"The page [[{self.pagename}]] currently does not exist. It must be created manually before the script can update it.")
            return
    ...
    def _get_user_name(self, row: Tag) -> str:
        ...
        raise Exception(f"Unexpected data in the table - could not find a user name in row '{row}'.")
