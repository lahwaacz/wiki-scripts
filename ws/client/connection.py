...
class Connection:
    ...
    def __init__(self, api_url: str, index_url: str, session: httpx.Client, timeout: int = 60):
    ...
    def from_argparser(cls, args: argparse.Namespace) -> Self:
        ...
        """
        session = Connection.make_session(max_retries=args.connection_max_retries, cookie_file=args.cookie_file)
        return cls(
            args.api_url,
            args.index_url,
            session=session,
    ...
    def call_api(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise APIJsonError("Failed to decode server response. Please make sure that the API is enabled on the wiki and that the API URL is correct.")
