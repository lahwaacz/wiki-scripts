...
class ExtlinkReplacements(CheckerBase, URLStatusChecker):
    ...
    def check_extlink_to_wikilink(
        self, wikicode: Wikicode, extlink: ExternalLink, url: str
    ) -> bool:
    ...
    def check_extlink_replacements(
        self, wikicode: Wikicode, extlink: ExternalLink, url: str
    ) -> bool:
        repl = None
        ...
        for (
            ...
            if (
                isinstance(text_cond, self.ExtlinkBehaviour)
                and extlink.title is not None
            ):
                continue
            ...
            if match:
                if extlink.title is None:
                    ...
                    if isinstance(text_cond, str) and re.fullmatch(
                        text_cond.format(*groups), alt_text, text_cond_flags
                    ):
                        repl = replacement.format(*match.groups(), extlink.title)
                        break
                    else:
                        logger.warning(
                            f"external link that should be replaced, but has custom alternative text: {extlink}"
                        )
    ...
    def check_url_replacements(
        ...
        for edit_summary, url_regex, url_replacement in self.url_replacements:
            ...
            if match:
                ...
                new_url = httpx.URL(
                    template.render(m=match.groups(), **match.groupdict())
                )
                ...
                if new_url.host == "gitlab.archlinux.org":
                    ...
                    if len(response.history) > 0:
                        if response.url.path.endswith("/master"):
                            ...
                            logger.warning(
                                f"URL not replaced (Gitlab redirected to a master branch): {url}"
                            )
                            return False
                ...
                extlink.url = str(new_url)  # type: ignore
    ...
    def check_http_to_https(
        self, wikicode: Wikicode, extlink: ExternalLink, url: httpx.URL | str
    ) -> None:
        url = httpx.URL(url)
    ...
    def update_extlink(
        self, wikicode: Wikicode, extlink: ExternalLink, summary_parts: list[str]
    ) -> None:
