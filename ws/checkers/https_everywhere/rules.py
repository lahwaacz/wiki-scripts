...
#import regex
import re as regex  # TODO: check that we don't use any additional functionality of `regex`
import socket
...
class Rule(object):
    ...
    def __init__(self, ruleElem):
        ...
        @param ruleElem: dict describing the <rule> element
        """
        self.fromPattern = ruleElem["from"]
        ...
        self.toPattern = regex.sub(r"\$(\d)", r"\\g<\1>", ruleElem["to"])
...
class Exclusion(object):
    ...
    def __init__(self, pattern):
        ...
        @param pattern: <exclusion> element from lxml tree
        """
...
class Test(object):
    ...
    def __init__(self, url):
        ...
        @param url: <test> element from lxml tree
        """
        self.url = url
...
class Ruleset(object):
    ...
    def _testConvert(elemList):
        return [Test(elem["url"]) for elem in elemList]
    ...
    #   attribute in the JSON dict,
    #   conversion function into value)
    _attrConvert = [
        ("name",        "name",         _strAttr),
        ("platform",    "platform",     _strAttr),
        ("defaultOff",  "default_off",  _strAttr),
        ("targets",     "target",       _targetAttrs),
    ...
    def __init__(self, element, filename):
        ...
        @param element: JSON dict corresponding to the <ruleset> element
        ...
        for (attrName, jsonAttr, conversion) in self._attrConvert:
            elems = element.get(jsonAttr)
            if elems:
