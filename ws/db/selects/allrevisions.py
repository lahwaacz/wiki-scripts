...
def sanitize_params(params):
    # MW incompatibility: arvgeneratexml is not supported (it is deprecated anyway)
...
def list(db, params=None, **kwargs):
    ...
    if {"expandtemplates", "parse", "section", "diffto", "difftotext", "difftotextpst", "contentformat",
        "generatetitles", "limit", "continue"} & set(params):
    ...
    # joins
    if "content" in prop:
        tail = tail.outerjoin(db.text, rev.c.rev_text_id == db.text.c.old_id)
        s.append_column(db.text.c.old_text)
    if "tags" in prop:
        tag = db.tag
        tgrev = db.tagged_revision
