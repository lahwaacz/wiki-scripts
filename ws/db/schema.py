...
def create_recomputable_tables(metadata):
    ...
    Table(
        ...
        metadata,
        Column(
            "wspc_page_id", Integer, ForeignKey("page.page_id", ondelete="CASCADE", deferrable=True, initially="DEFERRED"), primary_key=True, nullable=False
        ),
...
def create_multimedia_tables(metadata):
    image = Table(
        ...
        Column(
            "img_major_mime",
            Enum("unknown", "application", "audio", "image", "text", "video", "message", "model", "multipart", "chemical"),
            nullable=False,
