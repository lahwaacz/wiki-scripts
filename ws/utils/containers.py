...
def bisect_find[T: "SupportsDunderLT"](data_list: Sequence[T], key: T, index_list: Sequence[T] | None = None) -> T:
    """
...
def bisect_insert_or_replace[T: "SupportsDunderLT"](
...
def gen_nested_values(indict: dict | list | tuple, keys: list | None = None) -> Generator[tuple[list, Any]]:
    """
...
def parse_timestamps_in_struct(struct: dict | list) -> None:
    ...
    for keys, value in gen_nested_values(struct):
        if isinstance(value, str):
            ...
            elif len(value) == 20 and value[4] == "-" and value[7] == "-" and value[10] == "T" and value[13] == ":" and value[16] == ":" and value[19] == "Z":
                try:
                    ts = parse_date(value)
                except ValueError:
                    continue
